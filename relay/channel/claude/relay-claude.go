	...
	for i, message := range textRequest.Messages {
		...
		if lastMessage.Role == message.Role && lastMessage.Role != "tool" {
			if lastMessage.IsStringContent() && message.IsStringContent() {
				...
				// 非纯文本的连续同角色消息，用 ParseContent 合并 MediaContent 数组
				lastParts := lastMessage.ParseContent()
				currParts := fmtMessage.ParseContent()
				if len(lastParts) > 0 && len(currParts) > 0 {
					merged := append(lastParts, currParts...)
					fmtMessage.SetMediaContent(merged)
					formatMessages = formatMessages[:len(formatMessages)-1]
				}
			}
		}
